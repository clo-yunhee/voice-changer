...
static std::pair<double, double> upperLowerBounds(const std::vector<double>& P)
{
...
static std::vector<std::complex<double>> initRoots(const std::vector<double>& P)
{
    const int degree = static_cast<int>(P.size()) - 1;
    ...
    static std::uniform_real_distribution<> angle(0, 2 * PI);
...
static std::vector<std::complex<double>> evaluatePolyDer(const std::vector<double>& P, const std::complex<double>& x, int numberOfDerivatives)
{
    const int degree = static_cast<int>(P.size()) - 1;
...
std::vector<std::complex<double>> vc::model::solveRoots(
